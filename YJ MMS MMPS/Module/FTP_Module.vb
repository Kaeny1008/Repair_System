...
Module FTP_Module
    ...
    '임시로 고정...
    'Public ftpUrl As String = "ftp://192.168.0.254:21"
    'Public ftpPort As Integer = 21
    'Public ftpID As String = "yujin_ftp"
    'Public ftpPassword As String = "Yujin_ftp!"
    Public ftpUrl As String = "ftp://" & serverIP & ":" & registryEdit.ReadRegKey("Software\Yujin\MMS\FTP", "ftpPort", "1052")
    Public ftpPort As Integer = registryEdit.ReadRegKey("Software\Yujin\MMS\FTP", "ftpPort", "1052")
    Public ftpID As String = registryEdit.ReadRegKey("Software\Yujin\MMS\FTP", "ftpID", "yujin_ftp")
    Public ftpPassword As String = registryEdit.ReadRegKey("Software\Yujin\MMS\FTP", "ftpPassword", "Yujin_ftp!")
    ...
    Public Sub ftpFolderMake(ByVal ftpURL As String)
        ...
        Catch ex As UriFormatException
            MsgBox(ex.Message, MsgBoxStyle.Information, "FTP 폴더만들기")
    ...
    Public Function ftpFolderSearch(ByVal ftpURL As String) As String
        ...
        Try
            ...
            listRequest = Nothing
        Catch ex As UriFormatException
            MsgBox(ex.Message, MsgBoxStyle.Information, "FTP 폴더검색")
        Catch ex As WebException
            MsgBox(ex.Message, MsgBoxStyle.Information, "FTP 폴더검색")
        Finally
    ...
    Public Sub ftpFileUpload(ByVal ftpURL As String, ByVal FileName As String)
        ...
        Dim i As Double
        ProgressbarEffect.Show()
        ProgressbarEffect.BringToFront()
        ProgressbarEffect.PBar.Maximum = fileInf.Length
        ProgressbarEffect.PBar.Value = 1
        ...
        While (iLength <> 0) '전송할 파일 크기만큼 1024씩 전송
            ...
            ProgressbarEffect.PBar.Value += iLength
            ProgressbarEffect.PbarPercent.Text = Format(ProgressbarEffect.PBar.Value * 100 / _
                                                        ProgressbarEffect.PBar.Maximum, "0#") & " % 보내는 중...."
        End While

        ProgressbarEffect.Close()
    ...
    Public Sub ftpFileDownload(ByVal ftpURL As String, ByVal filePath As String, ByVal fileName As String)
        ...
        If My.Computer.FileSystem.FileExists(localPath_download & fileName) Then
            ...
            Catch ex As Exception
                MsgBox(ex.Message, MsgBoxStyle.Critical, "YUJIN Manufacturing Management System")
            End Try
        End If
        ...
        Try
            ...
            ProgressbarEffect.PBar.Maximum = responseFileDownload.ContentLength
            ProgressbarEffect.PBar.Value = 1
            ...
            While bytesRead > 0
                ...
                Application.DoEvents()
                ProgressbarEffect.PBar.Value += bytesRead
                ProgressbarEffect.PbarPercent.Text = Format(ProgressbarEffect.PBar.Value *
                                                            100 /
                                                            ProgressbarEffect.PBar.Maximum, "0#") & " % 불러오는 중...."
            End While
            writeStream.Close()
            ProgressbarEffect.Close()

        Catch exceptionObj As Exception
            MsgBox(exceptionObj.Message.ToString(), MsgBoxStyle.Critical, "FTP 다운로드")
        Finally
            requestFileDownload = Nothing
            responseFileDownload = Nothing
            ProgressbarEffect.Dispose()
        End Try
    ...
    Public Sub ftpFileDelete(ByVal ftpURL As String, ByVal FileName As String)
        ...
        Catch exceptionObj As Exception
            MsgBox(exceptionObj.Message.ToString(), MsgBoxStyle.Critical, "FTP 파일삭제")
        Finally
    ...
    Public Sub ftpFolderDelete(ByVal ftpURL As String)
        ...
        Try
            ...
            'Console.WriteLine("Delete status: {0}", responseFileDelete.StatusDescription)
        Catch exceptionObj As Exception
            MsgBox(exceptionObj.Message.ToString(), MsgBoxStyle.Critical, "FTP 폴더삭제")
        Finally
            responseFileDelete = Nothing
